...
"""
Copied from the original objax implementation, but modified bc of
hardcoded maxpool op 🤦‍♂️
...
class ResNetV2Block(objax.Module):
    ...
    def __init__(
        ...
        if self.stride > 1:
            self.shortcut = functools.partial(
                objax.functional.max_pool_2d, size=1, strides=self.stride
            )
        else:
    ...
    def __repr__(self) -> str:
        repr_str = "ResBlock:"
        for layer_str in [f"{str(l1)}\n\t{str(l2)}" for l1, l2 in self.layers]:
            repr_str += f"\n\t{layer_str}"
        return f"{repr_str}"
...
class ResNetV2(objax.nn.Sequential):
    ...
    def __init__(
        ...
        ops.extend(
            [
                normalization_fn(nout),
                activation_fn,
                lambda x: x.mean((2, 3)),
                objax.nn.Linear(nout, num_classes, w_init=objax.nn.init.xavier_normal),
            ]
        )
        super().__init__(ops)
...
def convert_bn_layer(keras_layer, objax_layer):
    ...
    objax_layer.gamma = objax.TrainVar(
        jn.array(keras_layer.__dict__["gamma"].numpy()).reshape(shape)
    )
    objax_layer.beta = objax.TrainVar(
        jn.array(keras_layer.__dict__["beta"].numpy()).reshape(shape)
    )
    objax_layer.running_mean = objax.StateVar(
        jn.array(keras_layer.__dict__["moving_mean"].numpy()).reshape(shape)
    )
    objax_layer.running_var = objax.StateVar(
        jn.array(keras_layer.__dict__["moving_variance"].numpy()).reshape(shape)
    )
...
def convert_conv_or_linear_layer(keras_layer, objax_layer):
    ...
    if keras_layer.__dict__.get("bias") is not None:
        ...
        objax_layer.b = objax.TrainVar(
            jn.array(keras_layer.__dict__["bias"].numpy()).reshape(bias_shape)
        )
    else:
        objax_layer.b = None
...
def convert_keras_model(
    model_keras, model_objax, num_blocks: list, include_top: bool = True
):
    ...
    if include_top:
        convert_conv_or_linear_layer(
            model_keras.get_layer("predictions"), model_objax[10]
        )


def load_pretrained_weights_from_keras(
    arch: str, include_top: bool = True, num_classes: int = 1000
):
    ...
    assert (
        tf is not None
    ), "Please install tensorflow dependency to be able to load keras weights."
    assert arch in model_registry, f"Model weights does not exist for {arch}."
    assert (
        not include_top or num_classes == 1000
    ), "Number of classes should be 1000 when including top layer."

    model_keras = getattr(tf.keras.applications, arch + "V2")(
        include_top=include_top,
        weights="imagenet",
        classes=1000,
        classifier_activation="linear",
    )
    model_objax = objax.zoo.resnet_v2.__dict__[arch](
        in_channels=3,
        num_classes=num_classes,
    ...
    convert_keras_model(
        model_keras, model_objax, model_registry[arch]["num_blocks"], include_top
    )
    del model_keras
    return model_objax
