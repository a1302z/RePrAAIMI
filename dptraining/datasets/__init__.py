...
SUPPORTED_DATASETS = ("cifar10", "imagenet", "tinyimagenet", 'pneumoniamnist', 'mimic')

SUPPORTED_FFT = ("cifar10",)
SUPPORTED_NORMALIZATION = ("cifar10", "tinyimagenet", 'pneumoniamnist', 'mimic')
...
def make_dataset(config):
    ...
    elif config["dataset"]["name"].lower() == "mimic":
        train_ds, val_ds, test_ds = MIMICCreator.make_datasets(
            config, (train_tf, val_tf, test_tf)
        )
...
def make_loader_from_config(config):
    ...
    if config["dataset"]["name"].lower() == "cifar10":
        ...
        train_loader, val_loader, test_loader = MIMICCreator.make_dataloader(
            train_ds,
            val_ds,
            test_ds,
            train_loader_kwargs,
