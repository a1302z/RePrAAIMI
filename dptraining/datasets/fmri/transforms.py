...
class UnetDataTransform:
    ...
    def __call__(
        ...
        if self.size != 320:
            ...
            image_r = resize(image[..., 0], self.size)
            image_i = resize(image[..., 1], self.size)
            target_torch = resize(target_torch, self.size)
            image = torch.view_as_complex(torch.stack((image_r, image_i), -1))
            return image, target_torch

        return torch.view_as_complex(image).unsqueeze(0), target_torch.unsqueeze(0)
