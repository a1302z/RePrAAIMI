...
from dptraining.datasets.fmri.fft2c import ifft2c_new as ifft2c, fft2c_new as fft2c

# from dptraining.datasets.fmri.math import complex_abs
from dptraining.datasets.fmri.coil_combine import rss, rss_complex

from dptraining.datasets.fmri.subsample import MaskFunc
...
def normalize_instance(
    ...
    return normalize_std(data, std, eps), std
...
def resize_complex(image, size):
    ...
    image = torch.view_as_complex(torch.stack((image_r, image_i), -1))
    return image
...
class UnetDataTransform:
    ...
    def __call__(
        ...
        # normalize input
        image, std = normalize_instance(image, eps=1e-11)
        image = image.clamp(-6, 6)

        # normalize target
        if target is not None:
            # target_torch = to_tensor(target)
            target_torch = complex_center_crop(
                target, crop_size
            )  # TODO check how to transfer to complex data
            target_torch = normalize_std(target_torch, std, eps=1e-11)
            target_torch = target_torch.clamp(-6, 6)
        else:
            target_torch = torch.Tensor([0])

        if self.size != 320:
            image = resize_complex(image, self.size)
            target_torch = resize_complex(target_torch, self.size)
            return image, target_torch

        return torch.view_as_complex(image).unsqueeze(0), torch.view_as_complex(
            target_torch
        ).unsqueeze(0)
