...
def main(
    ...
    from dptraining.utils.train_ops import create_train_op_normal, create_train_op_accumulate, create_train_op_SAT
    ...
    with jax.checking_leaks():
        ...
        if config.DP.SAT:
            print("using SAT train_op")
            create_train_op_func = create_train_op_SAT
        elif config.DP.grad_acc_steps > 1:
            print(f"using gradient accumulation train_op with {config.DP.grad_acc_steps} steps")
            create_train_op_func = create_train_op_accumulate
        else:
            print("normal train_op")
            create_train_op_func = create_train_op_normal

        train_op = create_train_op_func(
            train_vars,
            loss_gv,
            opt,
            augment_op,
